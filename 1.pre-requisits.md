...
**********************************************************************************************************************************
# Verification 
## Verify your DR is in recovery mode or not
```
docker compose exec -T postgres psql -U postgres -d postgres -c "SELECT pg_is_in_recovery() AS is_standby;"
```
_**OUTPUT**_:
On **DR**: t (true)
On **DC**: f (false)

## On DC check connection status
...
```
## Verfy is standby connected + lag? DC
```
cd /root
docker compose exec -u postgres postgres psql -U alfresco -d postgres -c "
SELECT
...
FROM pg_stat_replication;"
```
## On STANDBY (DR): am I receiving WAL from primary?
```
cd /root
docker compose exec -u postgres postgres psql -U alfresco -d postgres -c "
SELECT
