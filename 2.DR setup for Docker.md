...
## DC (Primary) — check standby is connected + streaming
...
```
Output may like,
<img width="1343" height="67" alt="image" src="https://github.com/user-attachments/assets/13c816aa-8bdd-4d35-8e73-5e85f97b9fc0" />

### 2) Check replication slot (if you use slot dr_slot)
```
docker compose exec -T postgres psql -U alfresco -d postgres -c "
...
<img width="375" height="71" alt="image" src="https://github.com/user-attachments/assets/24279561-c1f5-4539-838d-12adae6e7ab9" />
...
### 1. SSH
```
ssh -p 44220 -i /home/ketan_gcloud/.ssh/id_ed25519 \
  -o StrictHostKeyChecking=accept-new \
  drsync@10.128.0.13 "whoami; hostname"
...
sudo rsync -aHAX --numeric-ids --delete --inplace --partial --info=progress2 \
  -e "ssh -p 44220 -i /root/.ssh/id_ed25519 -o StrictHostKeyChecking=accept-new" \
  --rsync-path="sudo rsync" \
  /root/volumes/data/alf-repo-data/contentstore/ \
  drsync@10.128.0.13:/root/volumes/data/alf-repo-data/contentstore/
```
